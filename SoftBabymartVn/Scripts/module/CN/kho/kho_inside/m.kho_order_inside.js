...
KHO_Order_Product.mKHO_Order_Inside = function () {
    var self = this;
    ...
    self.soft_KHO_Order_Inside_Child = ko.observableArray();
};
KHO_Order_Product.mKHO_Order_Inside_Child = function () {
    var self = this;
    ...
    self.Total_Inside = ko.observable(0);
    self.IdDvt = ko.observable();

    self.IdNPP = ko.observable();
    self.Total = ko.computed(function () {
       if (self.Total_Money() > 0 || self.Total_Inside()>0)
            return self.Total_Money() * self.Total_Inside();
        return 0;
    });
    self.Note = ko.observable();
